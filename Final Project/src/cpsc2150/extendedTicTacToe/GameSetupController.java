...
public class GameSetupController {
    ...
    public void processButtonClick(int rows, int cols, int players, int numWin) {
        String errorMsg = "";
        if (rows < IGameBoard.MIN_VALUE || rows > max_size) {
            errorMsg += "Rows must be between " + IGameBoard.MIN_VALUE + " and " + max_size;
        }

        if (cols < IGameBoard.MIN_VALUE || cols > max_size) {
            errorMsg += "Columns must be between " + IGameBoard.MIN_VALUE + " and " + max_size;
        }
        ...
        if (numWin < IGameBoard.MIN_WIN) {
            errorMsg += "Number to win must be at least " + IGameBoard.MIN_WIN;
        }
